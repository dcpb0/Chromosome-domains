...
% ------------------------------------------------------------------------------------------------------------------------------------------
% modified from Ahilya N. Sawh, by DCPB
% 16.02.2021
%
%
%% ------------------------------------------------------------------------------------------------------------------------------------------
...
if SegInfo == 1;
    ...
    Thresh405= 950%TO TEST
    NucbinaryThresh = 900

    I3 = I2;
    I3(I3<150) = 0; %80&100 Increase if the DAPI is strong. Inical value 100. %50
...
%     title(['colored watershed label matrix nuclei', ' NucbiTh ' , num2str(NucbinaryThresh)])
   ...
   close all 
    ...
    if primarytosegment_647 == 1
        ...
        I3 = I2;
        I3(I3<30) = 0;%10 lower when primary is weak
    ...
    %save(['SegInfo/Num' num2str(SampleNum) '_age_' num2str(age) '_FOV_' num2str(FOV) ])
    display('seg done')
end

%% tracing with the seg infor saved
...
if Traces == 1;
    ...
    save_traces = 1;
    TracewithSegInfo = 1;
    if TracewithSegInfo == 1;
        for Num= 18:36
            for Age = 2:147
                for FOV = 1:2;
                        display(Num)

                    clearvars -except save_traces TracewithSegInfo Num Age FOV bfmatlabpath pathString
                    if exist (['SegInfo/Num' num2str(Num) '_age_' num2str(Age) '_FOV_' num2str(FOV) '.mat'], 'file') == 2
                        load(['SegInfo/Num' num2str(Num) '_age_' num2str(Age) '_FOV_' num2str(FOV) '.mat']);

                        % load and crop results file to FOV

                        load(['result' num2str(Num) '.mat']);
